...
export default function TerraFlashCardGame() {
  ...
  const fetchQuestions = async (instrument: InstrumentType) => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:3001/api/terra/${instrument.toLowerCase()}`);
  ...
  const handleAnswerSelect = (answer: string) => {
    ...
    setGameStats(prev => ({
      ...
      bestStreak: Math.max(prev.bestStreak, isCorrect ? prev.currentStreak + 1 : prev.currentStreak),
    }));
  };
        ...
        <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 pt-20">
          ...
          >
            ...
            <p className="text-xl text-gray-300 mb-8 max-w-2xl mx-auto">
              Test your knowledge about Terra&apos;s instruments with interactive flash cards
            </p>
          </motion.div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {instruments.map((instrument, index) => (
                <motion.button
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${instrument.color.replace('/20', '/10')}, rgba(255,255,255,0.02))`,
                    borderImage: `linear-gradient(135deg, ${instrument.borderColor}) 1`,
                  }}
                >
                  {/* Glow effect */}
                  <div className={`absolute inset-0 bg-gradient-to-r ${instrument.borderColor} rounded-2xl blur-xl opacity-0 group-hover:opacity-30 transition-opacity duration-300`}></div>
                  
                  <div className="relative z-10">
                    <div className="text-4xl mb-4 text-center">{instrument.icon}</div>
        ...
        <div className="relative z-10 min-h-screen flex flex-col px-4 sm:px-6 lg:px-8 pt-20 pb-8">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="text-center">
              <h2 className="text-2xl font-bold text-blue-300">{selectedInstrument}</h2>
              <p className="text-sm text-gray-400">Question {currentQuestionIndex + 1} of {questions.length}</p>
            </div>
            
            <div className="text-right">
              <p className="text-sm text-gray-400">Score</p>
              <p className="text-xl font-bold text-green-400">{gameStats.correct}/{gameStats.total}</p>
            </div>
          </div>
            ...
            <div className="w-full max-w-2xl mx-auto perspective-1000">
              ...
              >
                ...
                <div 
                  ...
                  style={{
                    background: "linear-gradient(135deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.05) 50%, rgba(59,130,246,0.1) 100%)",
                  ...
                  <div className="text-center mb-6">
                    {selectedAnswer === currentQuestion.correct ? (
                      ...
                      <div className="mb-4">
                        ...
                        <p className="text-white">The correct answer was: <span className="font-bold text-green-400">{currentQuestion.correct}</span></p>
                      </div>
                    )}
                  </div>
          ...
          >
            ...
            >
              <div className="mb-8">
                ...
                <h2 className="text-2xl text-blue-300 mb-2">{selectedInstrument}</h2>
              </div>
              ...
              <div className="mb-8">
                ...
                <p className="text-2xl text-gray-300 mb-2">{gradeInfo.message}</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
                <div className="bg-white/10 rounded-xl p-4">
                  <div className="text-3xl font-bold text-green-400">{gameStats.correct}</div>
                  <div className="text-sm text-gray-400">Correct</div>
                </div>
                <div className="bg-white/10 rounded-xl p-4">
                  <div className="text-3xl font-bold text-red-400">{gameStats.total - gameStats.correct}</div>
                  <div className="text-sm text-gray-400">Incorrect</div>
                </div>
                <div className="bg-white/10 rounded-xl p-4">
                  <div className="text-3xl font-bold text-blue-400">{gameStats.bestStreak}</div>
